...
class ActionOutcome(str, enum.Enum):
    """
    Outcome status for agent actions.
    
    Lifecycle:
    PENDING → SENT → RESOLVED (success) or EXPIRED (no response)
    PENDING → FAILED (delivery error)
    """
    PENDING = "pending"       # Action created, not yet executed
    SENT = "sent"             # Notification delivered successfully
    FAILED = "failed"         # Delivery failed
    RESOLVED = "resolved"     # Patient took desired action (showed up, submitted reading)
    EXPIRED = "expired"       # Window passed without resolution (48h+)
...
class AgentAction(BaseModel):
    ...
    # Outcome Tracking (for idempotency and closed-loop feedback)
    outcome: Mapped[ActionOutcome] = mapped_column(
        Enum(ActionOutcome), 
        default=ActionOutcome.PENDING, 
        index=True
    )
    outcome_detected_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
