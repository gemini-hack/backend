...
async def list_agent_actions(
    ...
    query = select(AgentAction).options(selectinload(AgentAction.patient)).join(AgentAction.patient).where(
    ...
    for action in actions:
        ...
        if action.patient:
            item.patient_name = f"{action.patient.first_name} {action.patient.last_name}"
            item.patient_uid = action.patient.patient_uid
...
async def list_resolved_actions(
    ...
    """
    query = select(AgentAction).options(selectinload(AgentAction.patient)).where(
        AgentAction.organization_id == user.organization_id,
    ...
    for a in actions:
        ...
        if a.patient:
            item.patient_name = f"{a.patient.first_name} {a.patient.last_name}"
            item.patient_uid = a.patient.patient_uid
        action_responses.append(item.model_dump())
...
async def list_alerts(
    ...
    query = select(Alert).options(selectinload(Alert.patient)).join(Alert.patient).where(
